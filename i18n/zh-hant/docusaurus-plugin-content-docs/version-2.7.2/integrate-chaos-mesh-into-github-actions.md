...
本文件描述如何使用 Chaos Mesh 整合 chaos-mesh-action 來自訂持續整合 (CI)。這能協助您在產品發布前，辨識系統開發中引入的問題。

chaos-mesh-action 是一個已在 [GitHub Marketplace](https://github.com/marketplace/actions/chaos-mesh) 發布的 GitHub action。其原始碼也位於 [GitHub](https://github.com/chaos-mesh/chaos-mesh-action)。
...
配備 GitHub Action 後，Chaos Mesh 可以輕鬆整合到您的日常開發和測試中，確保所有提交到 GitHub 的程式碼至少通過測試而沒有錯誤，且不影響當前邏輯。下圖顯示 chaos-mesh-action 整合到 CI 工作流程中：

![chaos-mesh-action 整合到 CI 工作流程中](./img/chaos-mesh-action-integrate-in-the-ci-workflow.png)

## 在 GitHub 工作流程中使用 chaos-mesh-action

chaos-mesh-action 適用於 GitHub 工作流程。GitHub 工作流程是可配置的自動化流程。您可以在儲存庫中設定 GitHub 工作流程以建置、測試、打包、發布或部署任何 GitHub 專案。要將 Chaos Mesh 整合到您的 CI 中，請遵循以下流程：
...
例如，我們可以設計一個簡單的測試工作流程。可以包含以下步驟：

1. 在 Kubernetes 叢集中建立兩個 Pod。

2. 從一個 Pod 傳送 ping 請求到另一個 Pod。

3. 使用 Chaos Mesh 注入網路延遲故障，以測試 ping 指令是否受到影響。

### 步驟 2：建立工作流程

設計完工作流程後，請按照以下步驟建立工作流程。
...
本質上，工作流程是一個循序的自動化作業配置。請注意，以下作業在單一檔案中配置。為了清楚說明，腳本被拆分為不同的工作組，如下所示：

- 設定工作流程名稱和觸發規則。

  將工作流程命名為 "Chaos"。當您提交程式碼或建立一個指向 master 分支的 pull request 時，此工作流程將被觸發。
...
- 安裝 CI 相關環境。
  ...
  此配置指定了操作系統 (Ubuntu) 並使用 helm/kind-action 創建一個 Kind 叢集。之後，它會列印叢集資訊。最後，它會簽出工作流程要存取的 GitHub 儲存庫。
...
- 部署一個應用程式。
  ...
  在以下範例中，此工作部署了一個建立兩個 Kubernetes Pod 的應用程式。
...
- 使用 Chaos Mesh 注入故障。
  ...
  使用 chaos-mesh-action，Chaos Mesh 會被自動安裝並注入故障。您只需準備混沌實驗的配置並取得其 base64 編碼值。若您想對 Pod 注入網路延遲，可以使用以下配置範例：
  ...
  使用以下指令取得上述混沌實驗配置檔案的 base64 編碼值：
...
- 驗證系統的正確性。
  ...
  在此作業中，工作流程從一個 Pod 發送 ping 請求到另一個 Pod，並觀察網路延遲。
...
一旦工作流程建立完成，您可以透過向主分支提交拉取請求來觸發它。當工作流程執行完畢後，作業驗證的輸出將類似以下內容：
...
輸出顯示一系列 10 毫秒的延遲，每次延遲持續 5 秒（共 5 次）。這與透過 chaos-mesh-action 注入的混沌實驗配置相符。

## 後續步驟

目前 chaos-mesh-action 已應用於 [TiDB Operator](https://github.com/pingcap/tidb-operator)。透過在工作流程中注入 Pod 故障，您可以驗證 Operator 實例的重啟機制，確保當注入的故障隨機刪除 TiDB Operator 的 Pod 時，Operator 仍能正常運作。詳情請參閱 [TiDB Operator 工作流程頁面](https://github.com/pingcap/tidb-operator/actions?query=workflow%3Achaos)。
