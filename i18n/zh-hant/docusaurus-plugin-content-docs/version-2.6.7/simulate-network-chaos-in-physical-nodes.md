...
2. 傳送一個 `POST` HTTP 請求到 Chaosd 服務的 `/api/attack/process` 路徑：

   ```bash
   curl -X POST 172.16.112.130:31767/api/attack/process -H "Content-Type:application/json" -d '{fault-configuration}'
   ```

   在上面的指令中，您需要根據故障類型設定 `fault-configuration`。對應的參數請參考以下各故障類型的參數和範例。

:::note

執行實驗時，請記得記錄實驗的 UID。當您要結束對應 UID 的實驗時，需要傳送一個 `DELETE` HTTP 請求到 Chaosd 服務的 `/api/attack/{uid}` 路徑。
