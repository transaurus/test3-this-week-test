...
本文件說明如何使用 Chaosd 模擬磁碟故障。此功能可協助您模擬磁碟讀寫負載（透過 [dd](https://man7.org/linux/man-pages/man1/dd.1.html)）或磁碟填充（透過 [dd](https://man7.org/linux/man-pages/man1/dd.1.html) 或 [fallocate](https://man7.org/linux/man-pages/man1/fallocate.1.html)）。

## 使用命令列模式建立實驗

本節描述如何使用命令列模式建立磁碟故障實驗。

在建立實驗之前，您可以執行以下命令來查看 Chaosd 支援的磁碟故障類型：
